...
# 启动时加载任务状态
tasks = load_tasks()
# 存储正在处理的URL或文件标识，防止重复处理
processing_identifiers = set()
# 存储活跃的任务对象，用于控制和取消
active_tasks = {}
# 存储SSE连接，用于实时推送状态更新
sse_connections = {}
...
async def process_video(
    ...
    """
    处理视频（支持视频链接或本地文件上传），返回任务ID
    """
    try:
        # 验证输入：必须提供链接或文件中的一个
        if not url and not file:
            raise HTTPException(status_code=400, detail="必须提供视频链接或上传本地文件")
        if url and file:
            raise HTTPException(status_code=400, detail="只能提供视频链接或上传本地文件中的一种")
        
        # 生成唯一标识符（用于去重）
        identifier = url if url else f"file:{file.filename}"
        
        # 检查是否已经在处理相同的内容
        if identifier in processing_identifiers:
            # 查找现有任务
...
async def process_video_task(task_id: str, url: Optional[str], file_path: Optional[str], file_name: Optional[str], summary_language: str):
    ...
    try:
        ...
        if detected_language and translator.should_translate(detected_language, summary_language):
            ...
            logger.info(f"不需要翻译: detected_language={detected_language}, summary_language={summary_language}")
...
async def delete_task(task_id: str):
    ...
    task_filename = tasks[task_id].get("filename")
    identifier = task_url if task_url else f"file:{task_filename}" if task_filename else ""
    if identifier:
        processing_identifiers.discard(identifier)
...
async def get_active_tasks():
    ...
    processing_count = len(processing_identifiers)
    return {
        "active_tasks": active_count,
        "processing_identifiers": processing_count,
