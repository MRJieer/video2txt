class VideoTranscriber {
    ...
    initializeElements() {
        ...
        this.localVideoInput = document.getElementById('localVideoInput'); 
    ...
    bindEvents() {
        ...
        if (this.localVideoInput) {
            this.localVideoInput.addEventListener('change', (e) => {
                if (e.target.files && e.target.files.length > 0) {
                    this.videoUrlInput.value = ''; // 选中文件时，清空URL输入框
                    this.hideError();
                    console.log('选中的本地文件:', e.target.files[0].name);
                }
            });
        }
    ...
    async startTranscription() {
        ...
        const selectedFile = this.localVideoInput.files[0]; // 获取选中的文件
        const videoUrl = this.videoUrlInput.value.trim();    // 获取URL输入
        const summaryLanguage = this.summaryLanguageSelect.value; // 摘要语言
        ...
        // 逻辑3：既无文件也无URL → 提示错误
            ...
            this.showError("请上传视频文件或输入视频URL");
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    urlInput.addEventListener('focus', () => {
        if (!urlInput.value) {
            // 新增本地文件示例提示
            urlInput.placeholder = '例如: https://www.youtube.com/watch?v=... 或 D:\\test.mp4（本地文件）';
        }
    });
    ...
    urlInput.addEventListener('blur', () => {
        if (!urlInput.value) {
            // 模糊时提示支持本地文件
            urlInput.placeholder = '输入视频URL，或选择本地MP4/MOV/AVI文件...';
        }
    });
});
